...
- **Điểm yếu của RAGAS AnsRel — quan trọng:** M07 (câu trả lời sáo rỗng) và H02 (câu trả lời lạc đề về chuối) vẫn được chấm AnsRel = 0.790/0.701 — *khá cao*, vì AnsRel chỉ đo "câu trả lời có *giống dạng* đang trả lời câu hỏi" qua embedding, không kiểm tra grounding/đúng-sai (đó là việc của Faithfulness, vốn đã đúng chấm 0.000 cho cả hai). Tệ hơn: **A02 (prompt injection)** — agent tuân theo lệnh tấn công, RAGAS Faith = 0.000 (đúng, không grounded) nhưng AnsRel = 0.784 (cao) → **không metric eval nào trong cả 2 framework bắt được lỗi an toàn này**, vì cả hai chỉ đo chất lượng nội dung, không đo safety. Kết luận: AnsRel cần luôn đi kèm Faithfulness, và injection/jailbreak cần một guardrail/classifier riêng, không thể trông cậy vào eval metric.
